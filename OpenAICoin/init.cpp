 ...
 * between YinchengCoind, and YinchengCoin-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("YinchengCoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)"), YinchengCoin_CONF_FILENAME));
    if (mode == HelpMessageMode::YinchengCoinD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)"), YinchengCoin_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/YinchengCoin/YinchengCoin>";
    const std::string URL_WEBSITE = "<https://YinchengCoincore.org>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("YinchengCoin-loadblk");
...
/** Sanity checks
 *  Ensure that YinchengCoin is running in a usable environment with all
...
{
    // Make sure only a single YinchengCoin process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", YinchengCoin_CONF_FILENAME)).string());
    LogPrintf("Using at most %i automatic connections (%i file descriptors available)\n", nMaxConnections, nFD);

    // Warn about relative -datadir path.
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if YinchengCoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if YinchengCoin is started while in a temporary directory.\n",
