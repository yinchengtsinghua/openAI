...
const char * const YinchengCoin_CONF_FILENAME = "YinchengCoin.conf";
const char * const YinchengCoin_PID_FILENAME = "YinchengCoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "YinchengCoind -foo=bar
...
#else
    const char* pszModule = "YinchengCoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\YinchengCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\YinchengCoin
    // Mac: ~/Library/Application Support/YinchengCoin
    // Unix: ~/.YinchengCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "YinchengCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/YinchengCoin";
#else
    // Unix
    return pathRet / ".YinchengCoin";
#endif
#endif
}
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", YinchengCoin_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure YinchengCoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("YinchengCoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The YinchengCoin Core developers";
    }
