...
std::string HelpMessageCli()
{
    ...
    strUsage += HelpMessageOpt("-rpcwallet=<walletname>", _("Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to YinchengCoind)"));
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  YinchengCoin-cli [options] <command> [params]  " + strprintf(_("Send command to %s"), _(PACKAGE_NAME)) + "\n" +
                  "  YinchengCoin-cli [options] -named <command> [name=value] ... " + strprintf(_("Send command to %s (with named arguments)"), _(PACKAGE_NAME)) + "\n" +
                  "  YinchengCoin-cli [options] help                " + _("List commands") + "\n" +
                  "  YinchengCoin-cli [options] help <command>      " + _("Get help for a command") + "\n";
    ...
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", YinchengCoin_CONF_FILENAME));
...
static UniValue CallRPC(BaseRequestHandler *rh, const std::string& strMethod, const std::vector<std::string>& args)
{
    ...
    if (response.status == 0) {
        ...
        throw CConnectionFailed(strprintf("Could not connect to the server %s:%d%s\n\nMake sure the YinchengCoind server is running and that you are connecting to the correct RPC port.", host, port, responseErrorMessage));
        ...
        if (failedToGetAuthCookie) {
            throw std::runtime_error(strprintf(
                ...
                GetConfigFile(gArgs.GetArg("-conf", YinchengCoin_CONF_FILENAME)).string().c_str()));
        } else {
                ...
                if (!error.isNull()) {
                    ...
                    if (error.isObject())
                    {
                        ...
                        if (errCode.isNum() && errCode.get_int() == RPC_WALLET_NOT_SPECIFIED) {
                            strPrint += "\nTry adding \"-rpcwallet=<filename>\" option to YinchengCoin-cli command line.";
                        }
                    }
                } else {
                    // Result
